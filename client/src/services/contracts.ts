...
export async function getAllNFTS(signer: ethers.Signer): Promise<IPxl[]> {
  ...
  const groupedByToken = allNFTs.reduce((acc, nft) => {
  ...
  for (const tokenId in groupedByToken) {
    const group = groupedByToken[tokenId].sort(
      (a, b) => a.minted_at - b.minted_at
    );
...
// Get all NFTs owned or generated by a specific user address.
